...
import type { VisitorRealtimeEvent } from "../modules/realtime/realtime.types";

type VisitorConnectionAttachment = {
  kind: "visitor";
...
export class VisitorStream {
  ...
  private handleWebSocket(request: Request): Response {
    const conversationId = request.headers.get("x-supportly-conversation-id");
    const visitorId = request.headers.get("x-supportly-visitor-id");
    ...
    const attachment: VisitorConnectionAttachment = {
      kind: "visitor",
      conversationId,
      visitorId,
      connectedAt: nowIso(),
    ...
    sendJson(server, { type: "connected", connectionKind: "visitor", serverTime: nowIso() });

    return new Response(null, { status: 101, webSocket: client });
  }

  private async handleNotify(request: Request): Promise<Response> {
    const event = (await request.json().catch(() => null)) as VisitorRealtimeEvent | null;
  ...
  private broadcast(event: VisitorRealtimeEvent): void {
